...
namespace REVIREPanels.Graficas
{
    class PositionChartOld    
    {
        ...
        private bool isDrawTargetZone = true; //Por defeco se debe pintar la zona objetivo
        private bool isDrawTargetCircle = true; //Por defeco se debe pintar el circulo objetivo

        #endregion

        //*******************************Inicialización*********************************//
        //******************************************************************************//
        public PositionChartOld(Chart inChart, double trhold)
        {
        ...
        private void InitChart()
        {
            ...
            Series serieIdeal = chart.Series["SeriesIdeal"];
            serieIdeal.ChartType = SeriesChartType.Line;
            serieIdeal.Color = Color.Green;
        ...
        public void DrawTargets(EventRobot events)
        {
        ...
        public void SetIdealTrajectory(Vector2[] position)
        {
            //Metodo para mostrar las trajectorias ideales                     
            IEnumerable<float> axisXData = position.Select(x => x.X);
            IEnumerable<float> axisYData = position.Select(y => y.Y);
            ...
            //Dibujar linea ejemplo
            Vector2[] puntos = new Vector2[2];
            /*  puntos[0] = new Vector2(-1.5f, 21.5f);
              puntos[1] = new Vector2(1.5f, 21.5f);
             puntos[2] = new Vector2(1.5f, 0.0f);
             puntos[3] = new Vector2(-1.5f, 0.0f);
             puntos[4] = new Vector2(-1.5f, 21.5f);*/

            /*Vector2[] puntos = new Vector2[14];
            //Cuadrado
            puntos[0] = new Vector2(-10, 2.5f);
            puntos[1] = new Vector2(10, 2.5f);
            puntos[2] = new Vector2(10, -2.5f);
            puntos[3] = new Vector2(-10, -2.5f);
            puntos[4] = new Vector2(-10, 2.5f);
            //Octaedro
            puntos[5] = new Vector2(-10, 15);
            puntos[6] = new Vector2( 10, 15);
            puntos[7] = new Vector2( 17.5f, 7.5f);
            puntos[8] = new Vector2(17.5f, -7.5f);
            puntos[9] = new Vector2(10, -15);
            puntos[10] = new Vector2(-10, -15);
            puntos[11] = new Vector2(-17.5f, -7.5f);
            puntos[12] = new Vector2(-17.5f,  7.5f);
            puntos[13] = new Vector2(-10, 15);*/

            /*puntos[0] = new Vector2(4, 4);
            puntos[1] = new Vector2(4, -4);
            puntos[2] = new Vector2(-5, 10);*/

            //puntos[0] = new Vector2(-4, 1);
            //  puntos[1] = new Vector2(25, 0.8f);

            //Entrar salir, entrar y salir, ambas direcciones
            /*puntos[0] = new Vector2(-1.70f, 15.0f);
            puntos[1] = new Vector2(-1.40f, 14.5f);
            puntos[2] = new Vector2(-1.35f, 14.0f);
            puntos[3] = new Vector2(-1.70f, 13.5f);
            puntos[4] = new Vector2(-1.75f, 13.0f);*/

            //Probar entra a circunferencia
            puntos[0] = new Vector2(-8.0f, 20.0f);
            puntos[1] = new Vector2(0.0f, 20.0f);
            ...
            //puntos[0] = new Vector2(-5, 10);
            // puntos[1] = new Vector2(15, 8);
            //   puntos[2] = new Vector2(15, -5);
            /* puntos[3] = new Vector2(5, -15);
             puntos[4] = new Vector2(-10, -15);
             puntos[5] = new Vector2(-15, -5);
             puntos[6] = new Vector2(-15, 5);
             puntos[7] = new Vector2(-5, 8);*/


            IEnumerable<double> axisX = puntos.Select(x => (double)x.X);
            IEnumerable<double> axisY = puntos.Select(y => (double)y.Y);
            //   Update(axisX, axisY, true);

            // chart.Series["SeriesPos"].Points.AddXY(-5, 10);
            // chart.Series["SeriesPos"].Points.AddXY(9, 8);
        ...
        private void CreateZoneIdealTrajectory(int index, Vector2 init, Vector2 end, double hold)
        {
            ...
            Vector2 current = new Vector2(final.X / GetModulo(final), final.Y / GetModulo(final));
...
//
        ...
        public void SliceTrial(TrialPacket trial, DataRobot[] datarobot)
        {
            //Corta una parte de la señal
            double limitInf = trial.timeTarget[0];
            double limitSup = trial.timeTarget[2];

            List<DataRobot> datasliced = datarobot.ToList().FindAll(x => x.TimeStamp >= limitInf && x.TimeStamp <= limitSup);

            //Coge indices inicial y final            
            int indexInf = datarobot.ToList().FindIndex(x => x.TimeStamp == datasliced[0].TimeStamp);
            int indexSup = datarobot.ToList().FindIndex(x => x.TimeStamp == datasliced.Last().TimeStamp);

            //Obtener velocidad
            IEnumerable<double> dataVxr = datarobot.ToList().Select(x => x.Xpr);
            IEnumerable<double> dataVyr = datarobot.ToList().Select(x => x.Ypr);
            //  IEnumerable<double> dataVxr = datarobot.ToList().Select(x => x.Zpr);
            //  IEnumerable<double> dataVyr = datarobot.ToList().Select(x => x.Rpr);
            IEnumerable<double> dataTime = datarobot.ToList().Select(x => x.TimeStamp);

            //Extraer valores marcados por los indices
            int sizen = datasliced.Count();

            double[] sliceddataVxr = new double[sizen];
            double[] sliceddataVyr = new double[sizen];
            double[] slicedtimestamp = new double[sizen];
            double[] moduleddataVr = new double[sizen];

            Array.Copy(dataVxr.ToArray(), indexInf, sliceddataVxr, 0, sizen);
            Array.Copy(dataVyr.ToArray(), indexInf, sliceddataVyr, 0, sizen);
            Array.Copy(dataTime.ToArray(), indexInf, slicedtimestamp, 0, sizen);

            //Calcular modulo de la velocidad
            for (int i = 0; i < sizen; i++)
            {
                Vector2 vectorv = new Vector2((float)sliceddataVxr[i], (float)sliceddataVyr[i]);
                moduleddataVr[i] = GetModulo(vectorv);
            }



            //   Update(slicedtimestamp, moduleddataVr, false);
            Update(sliceddataVxr, sliceddataVyr, false);
        ...
        public void Update(IEnumerable<double> xvalues, IEnumerable<double> yvalues, bool limits)
        {
            ...
            if (limits)
            {
                ...
                {
                    //Punto actual
                    currentPoint = new PointF((float)xvalues.ElementAt(i) * 100, (float)yvalues.ElementAt(i) * -100);
            ...
            else
            {
                for (int i = 0; i < xvalues.Count(); i++)
                {
                    ...
                    serie.Points.AddXY(xvalues.ElementAt(i) * 100, yvalues.ElementAt(i) * -100);
                    //chart.Series["SeriesPos"].Points.AddXY(xvalues.ElementAt(i) * 250, xvalues.ElementAt(i) * 100 + 50);
                }
            }
        ...
        public void UpdateWithoutIntersect(IEnumerable<double> xvalues, IEnumerable<double> yvalues, bool limits)
        {
            //Actualizar grafica comprobando si se tiene que señalar las zonas objetivo
            Series serie = chart.Series["SeriesPos"];
        ...
        private bool IsInQuadrilateral(PointF[] coefficients, PointF point, PointF[] circles)
        {
            ...
            if (!float.IsNaN(point.X) && !float.IsNaN(point.Y))
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    float distancia = GetModulo(vector);
        ...
        private double PEscalar(PointF p0, PointF p1)
        {
        ...
        private float GetModulo(Vector2 vect)
        {
