...
namespace REVIREPanels.Estadisticas.Graficas
{
    class SpeedChart
    {
        ...
        public SpeedChart(Chart inChart)
        {
        ...
        public void Update(IEnumerable<double> xvalues, IEnumerable<double> yvalues, IEnumerable<double> tvalues, float samples)
        {
            ...
            using (DataTable dt = new DataTable("RobotData"))
            {
                ...
                for (int i = 0; i < indices.Count(); i++)
                {
                    ...
                    row["X"] = tvalues.ElementAt(index);
                    row["Y"] = Math.Sqrt(xvalues.ElementAt(index) * xvalues.ElementAt(index) + yvalues.ElementAt(index) * yvalues.ElementAt(index));
                    dt.Rows.Add(row);
                }
